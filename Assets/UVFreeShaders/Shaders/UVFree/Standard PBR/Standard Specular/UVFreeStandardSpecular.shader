...
Shader "UVFree/StandardSpecular/Single-Texture Non-Batching" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandardSpecular o) {
			...
			fixed3 albedo = max(fixed3(0.0, 0.0, 0.0), tex.rgb * _Color.rgb);
			...
			fixed4 sg = fixed4(_SpecColor.rgb, _Glossiness);
			...
			//
				...
				o.Emission = ( 
					emissionX * IN.powerNormal.x
				  + emissionY * IN.powerNormal.y
				  + emissionZ * IN.powerNormal.z) * _EmissionColor.rgb;				
			#endif
